...
void genVariableAddress(Object* var) {
  // TODO
  int tmp = computeNestedLevel(VARIABLE_SCOPE(var));
  genLA(tmp,VARIABLE_OFFSET(var));
}

void genVariableValue(Object* var) {
  // TODO
  int tmp = computeNestedLevel(VARIABLE_SCOPE(var));
  genLV(tmp,VARIABLE_OFFSET(var));
}

void genParameterAddress(Object* param) {
  // TODO
  int tmp = computeNestedLevel(PARAMETER_SCOPE(param));
  genLA(tmp,PARAMETER_OFFSET(param));
}

void genParameterValue(Object* param) {
  // TODO
  int tmp = computeNestedLevel(PARAMETER_SCOPE(param));
  genLV(tmp,PARAMETER_OFFSET(param));
}

void genReturnValueAddress(Object* func) {
  // TODO
  int tmp = computeNestedLevel(FUNCTION_SCOPE(func));
  genLA(tmp,RETURN_VALUE_OFFSET);
}

void genReturnValueValue(Object* func) {
  // TODO
  int tmp = computeNestedLevel(FUNCTION_SCOPE(func));
  genLV(tmp,RETURN_VALUE_OFFSET);
}
...
void genProcedureCall(Object* proc) {
  // TODO
  int tmp = computeNestedLevel(proc->procAttrs->scope->outer);
  genCALL(tmp,proc->procAttrs->codeAddress);
}
...
void genFunctionCall(Object* func) {
  // TODO
  int tmp = computeNestedLevel(func->funcAttrs->scope->outer);
  genCALL(tmp,func->funcAttrs->codeAddress);
}
